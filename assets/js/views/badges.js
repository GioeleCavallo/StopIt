        ...
        renderBadgesGrid(grid, allBadges, unlockedBadges, category) {
            ...
            window.StopIt.Views.BadgesView.showDetails = (badgeId) => {
                ...
                const statusHtml = isUnlocked
                    ? `<div class="text-success font-bold text-lg">✅ Sbloccato il ${new Date(isUnlocked.unlockedAt).toLocaleDateString()}</div>`
                    : '<div class="text-secondary font-bold text-lg">🔒 Bloccato</div>';

                const helpText = isUnlocked ? "Hai ottenuto questo premio perché:" : "Come ottenerlo:";

                const content = `
                    <div class="text-center">
                        <div class="text-6xl mb-lg pulse-animation" style="font-size: 4rem;">${badge.icon}</div>
                        
                        <div class="card bg-primary-pale p-md mb-lg">
                            <h4 class="text-xs uppercase text-secondary mb-xs tracking-wide">${helpText}</h4>
                            <p class="font-bold text-lg text-primary-dark" style="line-height: 1.4;">${badge.description}</p>
                        </div>
                ...
                window.StopIt.Utils.Modals.alert(badge.name, content);
            };
        }
