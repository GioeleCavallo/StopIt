...
(function () {
    ...
    window.StopIt.Utils.BadgesEngine = {
        ...
        checkRevocation: function (userData, logs, plans, unlockedBadges) {
            console.log('🔍 Audit Revocation Start', { hasUser: !!userData, hasBadges: !!(unlockedBadges && unlockedBadges.length) });

            if (!unlockedBadges || !Array.isArray(unlockedBadges)) {
                console.warn('⚠️ Revocation skipped: unlockedBadges invalid');
                return [];
            }
            if (!userData) {
                console.warn('⚠️ Revocation skipped: userData invalid');
                return [];
            }
