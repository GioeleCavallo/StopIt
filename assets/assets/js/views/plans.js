    ...
    window.StopIt.Views.PlansView = class PlansView {
        ...
        attachEventListeners(container) {
            container.querySelector('#btn-add-plan').addEventListener('click', async () => {
                ...
                if (!triggerId || !action) {
                    window.StopIt.Utils.Notifications.show('Compila entrambi i campi!', 'warning');
                    return;
                }
                ...
                try {
                    ...
                    window.StopIt.Utils.Notifications.show('Piano aggiunto!', 'success');
                } catch (e) {
                    console.error(e);
                    window.StopIt.Utils.Notifications.show('Errore aggiunta piano', 'error');
                }
            });

            container.querySelectorAll('.btn-icon-delete').forEach(btn => {
                btn.addEventListener('click', async (e) => {
                    const id = e.target.closest('.btn-icon-delete').dataset.id;
                    window.StopIt.Utils.Modals.confirm(
                        'Elimina Piano',
                        'Vuoi davvero eliminare questo piano di emergenza?',
                        async () => {
                            await window.StopIt.State.deletePlan(parseInt(id));
                            this.render(container);
                            window.StopIt.Utils.Notifications.show('Piano eliminato.', 'info');
                        }
                    );
                });
            });
